...
func (h *Handler) HandleInsightGender(callbackQuery *tgbotapi.CallbackQuery, data string, historyManager *history.Manager, aiClient *ai.OpenAIClient) error {
	...
	diaryEntries, err := historyManager.GetUserDiary(userID, 0) // Получаем все записи
	if err != nil {
		// Если нет записей, предлагаем их создать
	...
	// Фильтруем записи по неделе и полу
	var weekEntries []history.DiaryEntry
	for _, entry := range diaryEntries {
		if entry.Week == weekNum {
			// Проверяем, содержит ли запись информацию о нужном поле
			entryLower := strings.ToLower(entry.Entry)
			if (gender == "male" && (strings.Contains(entryLower, "парень") || strings.Contains(entryLower, "мужчина") || strings.Contains(entryLower, "он"))) ||
			   (gender == "female" && (strings.Contains(entryLower, "девушка") || strings.Contains(entryLower, "женщина") || strings.Contains(entryLower, "она"))) ||
			   (!strings.Contains(entryLower, "парень") && !strings.Contains(entryLower, "девушка")) { // Общие записи
	...
	if len(weekEntries) == 0 {
		response := fmt.Sprintf("🔍 Персональный инсайт для %s %s (неделя %d)\n\n"+
			"📝 Не найдено записей в дневнике для %d недели.\n\n"+
			"Для создания персонального инсайта сделайте записи:\n"+
			"• Личные мысли и переживания\n"+
			"• Наблюдения о партнере\n"+
			"• Размышления об отношениях\n"+
			"• Выполнение упражнений недели\n\n"+
			"После записей вернитесь к инсайту!", 
			genderEmoji, genderName, weekNum, weekNum)
